...
class Escape:
    ...
    def move(self, destination):
        ...
        rect = self.macgyver.rect.move(self.maze_rect.x, self.maze_rect.y)
        ...
        if destination == self.guardian.position:
            if self.items.syringe in Items.backpack:
                ...
                self.screen.blit(self.guardian.image, rect)
        if game_status != "game lost":
            self.screen.blit(self.macgyver.image, rect)
        self.dirty_rects.append(rect)
        ...
        if game_status in ["game won", "game lost"]:
            ...
            text_position = self.blit_text(ending, text_position, font, pygame.Color("white"))
            text_position = self.blit_text(message, text_position, font, pygame.Color("orange"))
            self.screen.blit(self.maze_area, self.maze_rect)
            self.dirty_rects.append(self.maze_rect)
            Output.print_ending(self.CAPTION, self.ENDINGS[game_status])
        return game_status
                ...
                if event.type == pygame.QUIT:
                    ...
                    if command in ["exit game", "ok"] and game_status in ["game won", "game lost"]:
                        ...
                        if not self.collision_detected(destination):
                            game_status = self.move(destination) # MacGyver moves
                            # Update the screen
